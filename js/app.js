...
function startLoader() {
  ...
  function updateCounter() {
    if (currentValue < 100) {
      let increment = Math.floor(Math.random() * 10) + 5; // Increased increment
      currentValue = Math.min(currentValue + increment, 100);
      counterElement.textContent = currentValue;

      let delay = Math.floor(Math.random() * 90) + 15; // Reduced delay for faster update
...
function hideLoader() {
  gsap.to(".loader__count", { duration: 0.6, ease: 'power2.in', y: "100%", delay: 1.2 }); // Faster fade out
  gsap.to(".loader__wrapper", { duration: 0.6, ease: 'power4.in', y: "-100%", delay: 1.6 }); // Faster removal
  setTimeout(() => {
    document.getElementById("loader").classList.add("loaded");
  }, 2500); // Reduced total delay
}
...
function pageAppearance() {
  ...
  gsap.to(loadingItems, { 
    ...
    ease: 'power4',
    startAt: { y: 100 }, // Reduced initial offset
    y: 0,
    opacity: 1,
    delay: 0.6, // Reduced delay
    stagger: 0.03 // Faster stagger effect
  }, '>-=0.8');
  
  gsap.set(fadeInItems, { opacity: 0 });
  gsap.to(fadeInItems, { duration: 0.6, ease: 'none', opacity: 1, delay: 2.2 }); // Faster fade-in
}
